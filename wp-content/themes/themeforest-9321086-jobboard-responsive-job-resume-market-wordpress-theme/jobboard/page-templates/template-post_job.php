...
if (!current_user_can( 'manage_options' )) {
    ...
    if( jobboard_option('enable_package_job') == '1' && $is_limit && empty( $_GET['action'] ) ){
		...
		<?php
			...
			if( isset( $_GET['action'] ) && $_GET['action'] == 'edit' ){
				...
				$job = array(
					...
					'job_status' => get_post_meta( $edit->ID, '_jboard_job_status', true ),
				...
				<form id="post-job-form" class="frontend-form" action="" method="post" entype="multipart/form-data" role="form">
					...
					<div class="row">
						<div class="form-group col-sm-8">
							...
							<select name="job_company" id="job_company">
								<option value=""><?php echo '-- '.__( 'Select Company', 'jobboard' ).' --' ?></option>
								<?php
									$args = array(
										'post_type' => 'company',
						...
						<?php if (
							...
							( !empty($default['post_status']) && $default['post_status'] == 'publish' ) ) : ?>
						<div class="form-group col-sm-4">
							<label for="job_status"><?php _e( 'Job Status', 'jobboard' ); ?></label>

						  	<div class="btn-group" data-toggle="buttons">
							  <label class="btn btn-success<?php echo ($default['job_status']=='open')?' active':''; ?>">
								<input type="radio" name="job_status" id="job_status" value="open" autocomplete="off"> <?php _e('Open','jobboard') ?>
							  </label>
							  <label class="btn btn-danger<?php echo ($default['job_status']=='closed')?' active':''; ?>">
								<input type="radio" name="job_status" id="job_status" value="closed" autocomplete="off"> <?php _e('Close','jobboard') ?>
							  </label>
							</div>
						</div><!-- /.form-group -->
						<?php endif; ?>
