...
get_header();
	...
	while( have_posts() ){
		...
		if( !current_user_can( 'manage_options' ) || jobboard_option( 'resume_view_guest' ) != 1 ) {
	...
	<div class="container">
		<div class="row">
			...
			<div class="col-sm-8">
				...
				if( !is_user_logged_in() ){
					 ...
					 echo '<a href="#" class="btn btn-bookmark disabled"><i class="fa fa-star"></i>&nbsp;'. __( 'Login to Bookmark', 'jobboard' ) .'</a>';

				} else {
                        
					$resume_id = get_the_ID();
					$edit_resume_link = add_query_arg( array(
						'action'	=> 'edit',
						'jid'		=> $resume_id
					), esc_url( jobboard_get_permalink( 'post_resume') ) );
					...
					if( in_array( get_current_user_id(), $bookmark_array ) ){
							$btn_text = __( 'Unbookmark', 'jobboard' );
					} else {
				...
				?>
					<a href="<?php echo esc_url( $edit_resume_link ); ?>" class="btn btn-bookmark"><i class="fa fa-pencil-square-o"></i>&nbsp;<?php _e( 'Edit', 'jobboard' ); ?></a>
				<?php
					}else{
				?>
					<form id="bookmark-resume" method="post" action="<?php echo esc_url( admin_url('admin-ajax.php') ); ?>">
						...
						<button data-on-finish="<i class='fa fa-star'></i>&nbsp;<?php _e( 'Bookmark', 'jobboard' ); ?>" data-on-success="<i class='fa fa-star-o'></i>&nbsp;<?php _e( 'Unbookmark', 'jobboard' ); ?>" data-loading-text="<?php _e( 'Proccessing...', 'jobboard' ); ?>" id="bookmark-button" type="submit" name="submit" value="submit" class="btn btn-bookmark"><i class="fa fa-star"></i>&nbsp;<?php echo esc_attr( $btn_text ); ?></button>
                <?php 	$resume_file = get_post_meta( get_the_id(), 'jobboard_resume_file', true );
						if(!empty( $resume_file )){ ?>
						<a download target="_blank" href="<?php echo esc_url( $resume_file ); ?>" class="btn btn-resume"><?php _e( 'Download Resume', 'jobboard' ); ?></a>
				<?php }//endif; ?>
		...
		?>
			...
			<div class="candidate-details">
				<span><?php echo vp_metabox('jobboard_resume_mb.resume_professional_title'); ?> - <?php printf( __('%d years experience','jobboard'), vp_metabox( 'jobboard_resume_mb.year_experience') ); ?></span>
			</div>
			<div class="candidate-details">
				...
				<?php
					...
					printf( __( 'Member since %s ago', 'jobboard' ), human_time_diff( strtotime($user_date), current_time('timestamp') ) );
				?>
				</span>
	...
	<div class="container">
		<div class="row">
			...
			<div class="col-md-6">
				...
				<?php if (
					( is_user_logged_in() && jobboard_get_user_type(get_current_user_id()) != 'job_seeker' ) ||
					( $authorid == get_current_user_id() )
				) : ?>
				<div class="skills-container">
					<h3 class="skills-title" style="margin-bottom: 0px;">
						<?php _e( 'Proposed Monthly Salary', 'jobboard' ); ?> :
						<?php echo jobboard_price( vp_metabox( 'jobboard_resume_mb.proposed_monthly_sallary' ) ); ?>
					</h3>
					<?php if( $authorid == get_current_user_id() ): ?><small><?php _e('(this will only displayed to job lister)','jobboard') ?></small><?php endif; ?>
				</div>
				<br>
				<?php endif; ?>
