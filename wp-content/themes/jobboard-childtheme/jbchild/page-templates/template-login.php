...
<div id="page-title-wrapper" class="login-page-wrapper">
	<div class="container">
		<div class="row">
			...
			<div class="col-md-7">
				<h1 class="page-title">Submit your resume today</h1>
			</div>
		</div>
			...
			<div class="col-md-5">
				...
				if ( $error ) {
					...
					foreach ( $wp_error->get_error_codes() as $code ) {
						...
						if( $code == 'empty_password'){
							...
							echo apply_filters( 'jobboard_incorrect_password_msg', $error_str.__( 'The password you entered for the username <strong>'.$_POST['the_user_login'].'</strong> is incorrect.', 'jobboard' ).$lost_password_str );
						}
				...
				if(isset($_GET['action']) && $_GET['action'] == 'rp' ){
					...
					if ( isset($_POST['pass1']) && $_POST['pass1'] != $_POST['pass2'] )
						$errors->add( 'password_reset_mismatch', __( 'The passwords do not match.', 'jobboard' ) );
					...
					if ( ( ! $errors->get_error_code() ) && isset( $_POST['pass1'] ) && !empty( $_POST['pass1'] ) ) {
						...
						setcookie( $rp_cookie, ' ', time() - YEAR_IN_SECONDS, $rp_path, COOKIE_DOMAIN, is_ssl(), true );
						login_header( __( 'Password Reset', 'jobboard' ), '<p class="message reset-pass">' . __( 'Your password has been reset.', 'jobboard' ) . ' <a href="' . esc_url( wp_login_url() ) . '">' . __( 'Log in', 'jobboard' ) . '</a></p>' );
						login_footer();
						exit;
					}
				...
				<div id="login-form-wrapper" class="<?php echo $login_class; ?>">
					...
					<form id="login-form" action="" method="post">
						...
						<div id="answerme" class="answerme">
							<span id="the-question"><?php echo sprintf( __( 'What is %s =', 'jobboard' ), $math_login->getQuestion()); ?></span>
							<input class="form-control" type="text" name="human_answer" id="human_answer" required="required" placeholder="answer?" />
						</div>

						<input type="hidden" name="hash_answer" value="<?php echo $math_login->getAnswerHash(); ?>" />
						<input type="hidden" name="action" value="jobboard_proccess_login_form" />

						<?php $btn_label = __('Log in', 'jobboard'); ?>

						<div class="clearfix"></div>

						<button type="submit" name="user_submit" id="user_submit" value="1" class="btn btn-login"><?php echo $btn_label; ?></button>
						<div class="checkbox checkbox-info forgetmenot">
							<input name="rememberme" type="checkbox" id="rememberme" value="forever" class="styled">
							<label for="rememberme"><?php _e( 'Remember Me', 'jobboard' );?></label>
						</div>
