...
function jobboard_register_job_post_type() {
	...
	$args = apply_filters( 'jobboard_register_job_post_type_args', array(
		'label'               => __( 'job', 'jobboard' ),
		'description'         => __( 'Post Type Description', 'jobboard' ),
		'labels'              => apply_filters( 'jobboard_register_job_post_type_labels', $labels ),
...
if ( ! function_exists( 'jobboard_add_custom_job_admin_columns' ) ) :
function jobboard_add_custom_job_admin_columns( $columns ) {
	$new = array();
	foreach($columns as $key=>$value) {
		if ($key=='date') {
		 $new['applicant'] = __( 'Applicants', 'jobboard' );
		}
		$new[$key]=$value;
	}
	return $new;
}
add_filter( 'manage_job_posts_columns', 'jobboard_add_custom_job_admin_columns' );
endif;

if ( ! function_exists( 'jobboard_manage_custom_job_admin_columns' ) ):
function jobboard_manage_custom_job_admin_columns($column,$post_id) {
	switch ( $column ) {
		case 'applicant':
			$apps_args = array(
				...
				'post_status' => 'any',
				'posts_per_page' => -1,
				'meta_key' => '_jboard_applied_job',
				'meta_value' => $post_id
			);
			$q_apps = new WP_Query( $apps_args );
			echo $q_apps->post_count;
			wp_reset_postdata();
			break;
	}
}
add_action( 'manage_job_posts_custom_column', 'jobboard_manage_custom_job_admin_columns', 10, 2 );
endif;
...
function jobboard_register_job_category() {
	...
	$args = apply_filters( 'jobboard_register_job_category_args', array(
		'labels'                     => apply_filters( 'jobboard_register_job_category_labels', $labels ),
		'hierarchical'               => true,
		'public'                     => true,
		'show_ui'                    => true,
...
function jobboard_register_job_type() {
	...
	$args = apply_filters( 'jobboard_register_job_type_args', array(
		'labels'                     => apply_filters( 'jobboard_register_job_type_labels', $labels ),
		'hierarchical'               => true,
		'public'                     => true,
		'show_ui'                    => true,
		'show_admin_column'          => true,
...
function jobboard_register_job_region() {
	...
	$args = apply_filters( 'jobboard_register_job_region_args', array(
		'labels'                     => apply_filters( 'jobboard_register_job_region_labels', $labels ),
		'hierarchical'               => true,
		'public'                     => true,
		'show_ui'                    => true,
		'show_admin_column'          => true,
...
function jobboard_register_resume_post_type() {
	...
	$labels = apply_filters( 'jobboard_register_resume_post_type_labels', array(
	...
	$args = apply_filters( 'jobboard_register_resume_post_type_args', array(
		'labels'              => $labels,
