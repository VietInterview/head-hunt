...
if( isset( $_GET['action'] ) && $_GET['action'] == 'edit' ){
	if( empty( $_GET['jid'] ) || false === get_post_status( $_GET['jid'] ) ){
	 jobboard_404();
	}
	$jid = get_post( $_GET['jid'] );
	if( 'company' != $jid->post_type && get_current_user_id() != $jid->post_author ) {
		jobboard_404();
	}
}//endif;
	...
	<div class="container">
		<?php
			...
			if( isset( $_GET['action'] ) && $_GET['action'] == 'edit' ){
				...
				if ( ( empty($edit) || $edit->ID <= 0 ) || get_current_user_id() != $edit->post_author ) {
					wp_safe_redirect( jobboard_get_permalink( 'dashboard' ) );
					exit;
				}
			...
			<div class="col-md-8">
				<form id="post-company-form" class="frontend-form" action="" method="post" enctype="multipart/form-data" role="form">
					...
					if( isset( $_GET['action']) && $_GET['action'] == 'edit' ){
                        ...
                        if( !empty($company_service_data) ){

                            if( is_array($company_service_data) && array_key_exists('_jboard_company_service_group', $company_service_data) ){
					...
					<div class="company_part_data data_service">
						...
						if( isset( $_GET['action']) && $_GET['action'] == 'edit' ){
                            ...
                            if( !empty($company_client_data) ){

                                if( is_array($company_client_data) && array_key_exists('_jboard_company_client_group', $company_client_data) ){
...
if( isset( $_GET['action']) && $_GET['action'] == 'edit' ){
	...
	$company_portfolio_data_stored_url = get_post_meta( $edit->ID, '_jboard_company_portfolio_stored_url', true );
	...
	foreach( $companyportfoliodata as $portfolio ) :
		...
		if ( is_numeric( $portfolio['portfolio_image'] ) ) {
			...
			$attachment_id = $portfolio['portfolio_stored_image_id'];
			$portfolio_img_src = wp_get_attachment_image_src( $attachment_id, 'jobboard-company-portfolio' );
			$portfolio_img_src = $portfolio_img_src[0];
		} else {
			$attachment_id = jobboard_get_attach_id( $portfolio['portfolio_image'] );
			if ( $attachment_id > 0 ) {
				$portfolio_img_src = wp_get_attachment_image_src( $attachment_id, 'jobboard-company-portfolio' );
				$portfolio_img_src = $portfolio_img_src[0];
			} else {
		...
		$portfolio_url = !empty($portfolio['portfolio_url']) ? esc_url($portfolio['portfolio_url']) : '';
		$portfolio_img = !empty($portfolio_img_src) ? '<img src="'.esc_url($portfolio_img_src).'" />' : '';

		if( !empty($portfolio_img_src) ) { // Check if attachment id is not empty string
		?>
